$(document).ready(function () {
    ...
    $(window).resize(function () {
        $("#sidebar").css("min-height", 0); $("#sidebar").css("min-height", $(document).height() - $("#header").outerHeight() - $("#footer").outerHeight());
    });
    $('.loggedin').on('click', function () {
        $('.accountmenu').slideToggle(); $(this).toggleClass('active');
    });
    $('.actions').on('click', function () {
        $(this).parent().find('.actionmenu').slideToggle(); $(this).toggleClass('active');
    });
    $('.close').on('click', function () { $(this).parent().slideToggle(); });
    $('#sidebar h2 a').die();
    $('#sidebar h2 a').live('click',
    function (event) {
        ...
        if ($(this).find('img').attr('src').indexOf('up') > 0) {
            $(this).find('img').attr('src', '/public/images/icons/toggledown.png');
        } else {
            $(this).find('img').attr('src', '/public/images/icons/toggleup.png');
        }
    ...
    if ($.cookie('accountBalances') === undefined || $.cookie('accountBalances') === '1') {
        ...
        $('#accountBalances').prev().find('img').attr('src', '/public/images/icons/toggleup.png');
    }
    if ($.cookie('btcMarkets') === undefined || $.cookie('btcMarkets') === '1') {
        $('#btcMarkets').show();
        $('#btcMarkets').prev().find('img').attr('src', '/public/images/icons/toggleup.png');
    }
    if ($.cookie('ltcMarkets') === undefined || $.cookie('ltcMarkets') === '1') {
        $('#ltcMarkets').show();
        $('#ltcMarkets').prev().find('img').attr('src', '/public/images/icons/toggleup.png');
    }
    if ($.cookie('sidebarStats') === undefined || $.cookie('sidebarStats') === '1') {
        $('#sidebarStats').show();
        $('#sidebarStats').prev().find('img').attr('src', '/public/images/icons/toggleup.png');
    }
});
