...
namespace Dotpay.Front.YiSheng
{
    ...
    public class EasypayNotify
    {
        ...
        public EasypayNotify(SortedDictionary<string, string> inputPara, string notify_id, string partner, string key, string input_charset, string sign_type, string transport)
        {
            ...
            if (_transport == "https")
            {
                gateway = "https://www.easypay.com/cooperate/gateway.do?service=notify_verify";
            }
            else
            {
                //gateway = "http://10.68.76.19:28183/verify/notify?";
                gateway = "http://mapi.bhecard.com/verify/notify?";
            }
            ...
            sPara = EasypayFunction.ParaFilter(inputPara);          //过滤空值、sign与sign_type参数
            preSignStr = EasypayFunction.CreateLinkstring(sPara);   //获取待签名字符串（调试用）
            //获得签名结果
            mysign = EasypayFunction.BuildMysign(sPara, _key, _sign_type, _input_charset);

            //获取远程服务器ATN结果，验证是否是从易生支付服务器发来的请求
        ...
        public string Query(string partner, string out_trade_no,string input_charset, string sign_type,string sign)
        {
            ...
            //string send_url = "http://10.68.76.18:18183/query/payment?"; //查询接口地址
            string send_url = "http://mapi.bhecard.com/query/payment?"; //查询接口地址
            string query_url = send_url + "partner=" + _partner + "&_input_charset=" + _input_charset + "&out_trade_no=" + out_trade_no + "&sign=" + sign+ "&sign_type=" + sign_type;
            return Get_Http(query_url, 120000);
        }
        ...
        public string Refund(string partner, string input_charset, string sign_type, string out_trade_no, string orig_out_trade_no, string amount, string subject, string sign)
        {
            ...
            //string send_url = "http://10.68.76.18:18183/service/refund?"; //退款接口地址
            string send_url = "http://mapi.bhecard.com/service/refund?"; //退款接口地址
            string refund_url = send_url + "partner=" + _partner + "&_input_charset=" + _input_charset + "&sign_type=" + sign_type + "&out_trade_no=" + out_trade_no + "&orig_out_trade_no=" + orig_out_trade_no + "&amount=" + amount + "&subject=" + subject + "&sign=" + sign;
            return Get_Http(refund_url, 120000);
        }
