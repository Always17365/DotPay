...
{
    public class STRReceivePaymentTransactionMap : BaseClassMapping<STRReceivePaymentTransaction>
    {
        public STRReceivePaymentTransactionMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); });
            Property(x => x.UniqueID, map => { map.NotNullable(true); map.Unique(true); map.Length(25); });
            Property(x => x.TxID, map => { map.NotNullable(true); map.Index("IX_STELLAR_TXID"); map.Length(67); });
