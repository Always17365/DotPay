...
{
    public class FBCWithdrawMap : BaseClassMapping<FBCWithdraw>
    {
        public FBCWithdrawMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); map.Index("IX_FBC_WITHDRAW_USERID"); });
            Property(x => x.UniqueID, map => { map.NotNullable(true); map.Length(24); map.Index("IX_FBC_WITHDRAW_UNIQUE_ID"); });
            Property(x => x.AccountID, map => { map.NotNullable(true); });
            Property(x => x.TxID, map => { map.NotNullable(true); map.Length(67); map.Index("IX_FBC_WITHDRAW_TXID"); });
            Property(x => x.ReceiveAddress, map => { map.NotNullable(true); map.Index("IX_FBC_WITHDRAW_RECEIVE_ADDRESS"); });
