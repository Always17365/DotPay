...
{
    public class DOGEAccountMap : BaseClassMapping<DOGEAccount>
    {
        public DOGEAccountMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); map.Unique(true); });
            Property(x => x.Balance, map => { map.NotNullable(true); map.Precision(20); map.Scale(8); });
            Property(x => x.Locked, map => { map.NotNullable(true); map.Precision(20); map.Scale(8); });
