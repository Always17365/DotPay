...
{
    public class IFCAccountVersionMap : BaseClassMapping<IFCAccountVersion>
    {
        public IFCAccountVersionMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); });
            Property(x => x.AccountID, map => { map.NotNullable(true); map.Index("IX_IFC_ACCOUNT_VERSION_ACCOUNT_ID"); }); 
