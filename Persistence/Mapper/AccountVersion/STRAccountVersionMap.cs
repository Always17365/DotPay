...
{
    public class STRAccountVersionMap : BaseClassMapping<STRAccountVersion>
    {
        public STRAccountVersionMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); });
            Property(x => x.AccountID, map => { map.NotNullable(true); map.Index("IX_STR_ACCOUNT_VERSION_ACCOUNT_ID"); }); 
