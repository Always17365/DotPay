...
{
    public class NXTAccountVersionMap : BaseClassMapping<NXTAccountVersion>
    {
        public NXTAccountVersionMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UserID, map => { map.NotNullable(true); });
            Property(x => x.AccountID, map => { map.NotNullable(true); map.Index("IX_NXT_ACCOUNT_VERSION_ACCOUNT_ID"); }); 
