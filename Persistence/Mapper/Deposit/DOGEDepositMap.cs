...
{
    public class DOGEDepositMap : BaseClassMapping<DOGEDeposit>
    {
        public DOGEDepositMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UniqueID, map => { map.NotNullable(true); map.Length(22); map.Index("IX_DOGE_DEPOSIT_UNIQUEID"); });
            Property(x => x.UserID, map => { map.NotNullable(true); map.Index("IX_DOGE_DEPOSIT_USERID"); });
            ...
            Property(x => x.TxID, map => { map.NotNullable(true); map.Length(70); map.Index("IX_DOGE_DEPOSIT_TXID"); });
