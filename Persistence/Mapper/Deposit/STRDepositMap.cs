...
{
    public class STRDepositMap : BaseClassMapping<STRDeposit>
    {
        public STRDepositMap()
        {
            Id(u => u.ID, map => map.Generator(Generators.Identity));

            Property(x => x.UniqueID, map => { map.NotNullable(true); map.Length(22); map.Index("IX_STR_DEPOSIT_UNIQUEID"); });
            Property(x => x.UserID, map => { map.NotNullable(true); map.Index("IX_STR_DEPOSIT_USERID"); });
            ...
            Property(x => x.TxID, map => { map.NotNullable(true); map.Length(70); map.Index("IX_STR_DEPOSIT_TXID"); });
