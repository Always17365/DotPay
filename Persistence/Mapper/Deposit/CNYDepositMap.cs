...
namespace DotPay.Persistence
{
    public class CNYDepositMap : BaseClassMapping<CNYDeposit>
    {
        public CNYDepositMap()
        {
            ...
            Property(x => x.UniqueID, map => { map.NotNullable(true); map.Length(22); map.Index("IX_CNY_DEPOSIT_UNIQUEID"); });
            ...
            Property(x => x.FundSourceID, map => { map.NotNullable(true); map.Index("IX_CNY_DEPOSIT_FUNDSOURCE_ID"); });
            Property(x => x.FundExtra, map => { map.NotNullable(true); map.Length(100); });
