...
namespace DotPay.Domain
{
                 ...
                 IEventHandler<UserUnsignedRole>               //用户分配管理角色
    {
        ...
        public User(int commendBy, string email, string password, int timezone)
        {
            this.RaiseEvent(new UserRegisted(commendBy, email, password, timezone, this));
            if (commendBy > 0) this.Apply(new UserCommendSuccess(commendBy));
        }

        public User(int commendBy, string nickName, string password, OpenAuthType openAuthType)
        {
            this.RaiseEvent(new UserRegistedByOpenAuth(commendBy, nickName, password));
            if (commendBy > 0) this.Apply(new UserCommendSuccess(commendBy));
        }
        #endregion
